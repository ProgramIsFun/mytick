...
router.get('/backupable', async (req: AuthRequest, res: Response) => {
  try {
    const databases = await Database.find({
      userId: req.userId,
      backupEnabled: true,
    }).select('name type connectionUri backupRetentionDays backupFrequency');
...
router.post('/', async (req: AuthRequest, res: Response) => {
  try {
    const { name, type, connectionUri, host, port, database, backupEnabled, backupRetentionDays, backupFrequency, accountId, tags, notes } = req.body;
...
router.patch('/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const allowed = ['name', 'type', 'connectionUri', 'host', 'port', 'database', 'backupEnabled', 'backupRetentionDays', 'backupFrequency', 'accountId', 'tags', 'notes'];
